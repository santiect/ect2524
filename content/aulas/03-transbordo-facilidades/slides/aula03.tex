...
ampl.solve()
tempo = time.perf_counter() - t0          # tempo de execucao (s)

print(ampl.solve_result)                  # 'solved', 'limit', 'infeasible'...

custo = ampl.get_objective("custo").value()          # UB
lb    = ampl.get_value("custo.bestbound")             # limitante inferior
gap   = ampl.get_value("custo.relmipgap")             # gap relativo

x = ampl.get_variable("x").get_values().to_pandas()
y = ampl.get_variable("y").get_values().to_list()
	\end{lstlisting}
	\begin{itemize}
		\item \texttt{custo.bestbound} / \texttt{custo.relmipgap}: só
			depois de um \texttt{solve} de modelo inteiro;
		\item alternativa ao cronômetro: \texttt{ampl.get\_value("\_total\_solve\_time")}.
	\end{itemize}
\end{frame}

\begin{frame}[fragile]
	\frametitle{Rodar várias instâncias em laço}
	\begin{lstlisting}[language=Python]
import pandas as pd
...
for arq in arquivos:
    ...
    ampl.read("modelo.mod")       # (ou ampl.reset() antes de reusar)
    ampl.param["m"] = dados["m"]
...
print(tabela)
	\end{lstlisting}
	\begin{itemize}
		\item criar \texttt{AMPL()} de novo a cada volta evita erro de
			parâmetro/modelo ``já definido''.
	\end{itemize}
\end{frame}
