...
\begin{frame}
	...
	\begin{itemize}
		...
		\end{itemize}
		\item A escolha entre as duas é parte da tarefa -- e precisa de
			\textbf{justificativa} no contexto do exame.
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{enumerate}
			\item \textbf{Modelar}: identificar parâmetros, variáveis de
				decisão, função objetivo e restrições -- escolhendo e
				justificando uma das duas visões do objetivo;
			\item \textbf{Modelo em AMPL} (\texttt{facilidades.mod});
			\item \textbf{Código Python}: \texttt{ler\_dados} + script
				\texttt{amplpy} que resolve as \textbf{três instâncias} e
...
\begin{frame}
	...
	\begin{itemize}
		\item A instância 3 é bem maior: parte da tarefa é observar se ela
			é resolvida na otimalidade dentro do tempo limite, ou se
			sobra GAP;
